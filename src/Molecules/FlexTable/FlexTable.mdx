...
Row has the important feature of managing expand states. For it to know how to handle an out-of-the-box-expand it needs `expandItems` and/or `expandChildren`, alongside the parent `FlexTable` having `expandable = true`. If neither of `expandItems` or `expandChildren` are supplied, the chevron will be disabled.
