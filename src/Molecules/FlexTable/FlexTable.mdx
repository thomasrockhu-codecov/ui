...
The `fontSize` and `density` defined in the `FlexTable` component is inherited here. If the content does not fit within the `Header`, it will be truncated and produces a tooltip on hover via the molecule `TruncateWithToolTip`.
...
Table headers contain sorting, to customise a sortable table header you should use `Flexbox` to isolate the different items
...
`CellInlineContainer` is a Flexbox-wrapper that makes a `Flexbox` item able to truncate, also makes sure everything inside has correct vertical alignment.
...
In this example below we automatically get the `fontSize` and `density` from the table.

```jsx
const NumberCell: React.FC<{ columnId: string }> = React.memo(({ value, columnId }) => (
  <FlexTable.Cell columnId={columnId}>
    {({ fontSize, density }) => (
      <FlexTable.Cell.TextWrapper fontSize={fontSize} density={density}>
        <Number value={value} />
...
You could also skip using Render Props and supply the values for `fontSize` and `density` yourself in the example above.
