...
const VISIBLE_BY_CLICK = 'VISIBLE_BY_CLICK';
...
const GLOBAL_CLICK_OUTSIDE = 'GLOBAL_CLICK_OUTSIDE';
  ...
  states: {
    ...
    [VISIBLE_BY_CLICK]: {
      on: {
        ...
        [BLUR]: LEAVING_VISIBLE,
        [SELECT_WITH_KEYBOARD]: DISMISSED,
        [GLOBAL_CLICK_OUTSIDE]: LEAVING_VISIBLE,
      },
    },
    [LEAVING_VISIBLE]: {
...
function startLeavingVisibleTimer() {
  ...
  leavingVisibleTimer = window.setTimeout(() => transition(TIME_COMPLETE), LEAVE_TIMEOUT);
}
...
export function useTooltip<T extends HTMLElement>({
  ...
  let [isVisible, setIsVisible] = useState(
    DEBUG_STYLE
      ...
      : id === null
      ? false
      : context.id === id && (state === VISIBLE || state === VISIBLE_BY_CLICK),
  );
  ...
  useEffect(() => {
    return subscribe(() => {
      if (
        context.id === id &&
        (state === VISIBLE || state === VISIBLE_BY_CLICK || state === LEAVING_VISIBLE)
      ) {
        setIsVisible(true);
      } else {
        setIsVisible(false);
  ...
  useEffect(() => {
    ...
    function listener(event: KeyboardEvent) {
      if (
        ...
        (state === VISIBLE || state === VISIBLE_BY_CLICK)
      ) {
  ...
  useEffect(() => {
    ...
    function handleClickOutside(event) {
      console.log({ isVisible, isOutside: !ownRef?.current.contains(event.target) });
      if (state === VISIBLE_BY_CLICK && ownRef && !ownRef?.current.contains(event.target)) {
        console.log('LIFE SOLVED');
        transition(GLOBAL_CLICK_OUTSIDE);
      }
    }
 ...
 */
export const TooltipPopup = forwardRefWithAs<TooltipPopupProps, 'div'>(function TooltipPopup(
  {
 ...
 */
const TooltipContent = forwardRefWithAs<TooltipContentProps, 'div'>(function TooltipContent(
  {
    // TODO: Remove `ariaLabel` prop in 1.0 and just use `aria-label`
    ariaLabel,
    'aria-label': realAriaLabel,
...
type State = 'IDLE' | 'FOCUSED' | 'VISIBLE' | 'LEAVING_VISIBLE' | 'DISMISSED' | 'VISIBLE_BY_CLICK';

type MachineEventType =
  | 'BLUR'
  | 'FOCUS'
  | 'GLOBAL_MOUSE_MOVE'
  | 'GLOBAL_CLICK_OUTSIDE'
  | 'MOUSE_DOWN'
  | 'MOUSE_ENTER'
  | 'MOUSE_LEAVE'
  | 'MOUSE_MOVE'
